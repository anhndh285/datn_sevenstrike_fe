<!-- src/pages/product/thuoc_tinh/PhongCachChoiPage.vue -->
<template>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">{{ modal.mode === 'create' ? 'Thêm phong cách chơi' : 'Chỉnh sửa phong cách chơi' }}</div>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <template v-if="modal.mode === 'edit'">
            <label class="form-label ss-label">Mã phong cách</label>
            <input :value="form.maPhongCach" class="form-control ss-input-readonly" readonly disabled />
          </template>

          <label class="form-label ss-label ss-label-gap">Tên phong cách *</label>
          <input
            v-model="form.tenPhongCach"
            class="form-control"
...
function getId(row) {
  return row?.id ?? row?.idPhongCach ?? row?.id_phong_cach ?? row?.phongCachId ?? row?.phong_cach_id ?? null;
}
...
async function fetchAll() {
  ...
  try {
    ...
    console.error(e);
    showToast(getErrorMessage(e, "Không tải được danh sách phong cách chơi."), "error");
  } finally {
    loading.value = false;
  }
}
...
async function onToggle(row) {
  ...
  try {
    ...
    await Swal.fire({
      ...
      icon: "success",
      title: "Thành công!",
      text: "Đã cập nhật trạng thái phong cách chơi!",
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } catch (e) {
    ...
    await Swal.fire({
      ...
      icon: "error",
      title: "Thất bại!",
      text: getErrorMessage(e, "Không đổi được trạng thái phong cách chơi."),
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } finally {
...
function openEdit(row) {
  ...
  form.maPhongCach = getMa(row);
  form.tenPhongCach = getTen(row);
...
async function save() {
  ...
  if (!ten) {
    modal.error = "Vui lòng nhập tên phong cách.";
    return;
  }
  ...
  try {
    if (modal.mode === "create") {
      await phongCachChoiService.create({
        tenPhongCach: ten,
        trangThai: true,
        xoaMem: false,
      });
      ...
      modal.open = false;
      await fetchAll();
      showToast("Thêm phong cách chơi thành công!");
      return;
    }

    if (modal.editingId === undefined || modal.editingId === null) {
      modal.error = "Không tìm thấy ID phong cách để cập nhật.";
      return;
    }

    if (typeof phongCachChoiService.update === "function") {
      await phongCachChoiService.update(modal.editingId, {
        tenPhongCach: ten,
        trangThai: form.trangThai,
      });
    } else if (typeof phongCachChoiService.put === "function") {
      await phongCachChoiService.put(modal.editingId, {
        tenPhongCach: ten,
        trangThai: form.trangThai,
      });
    } else {
      throw new Error("Service chưa có hàm cập nhật phong cách chơi");
    }

    modal.open = false;
    await fetchAll();
    showToast("Cập nhật phong cách chơi thành công!");
  } catch (e) {
    console.error(e);
    modal.error = getErrorMessage(
      e,
      modal.mode === "create" ? "Thêm phong cách chơi thất bại." : "Cập nhật phong cách chơi thất bại."
    );
  } finally {
    saving.value = false;
  }
