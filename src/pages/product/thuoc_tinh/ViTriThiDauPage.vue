<!-- File: src/pages/product/thuoc_tinh/ViTriThiDauPage.vue -->
<template>
  <div class="ss-page ss-font">
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">Thêm vị trí thi đấu</div>
          <button class="btn btn-sm btn-outline-secondary" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <label class="form-label ss-label">Tên vị trí *</label>
          <input v-model="form.tenViTri" class="form-control" placeholder="Nhập tên..." />
        </div>
    ...
    <div v-if="success.open" class="ss-overlay">
      <div class="ss-success">
        ...
        <div class="ss-muted">Thêm vị trí thi đấu thành công!</div>
...
async function fetchAll() {
  ...
  try {
    ...
    console.error(e);
    alert("Không tải được danh sách vị trí thi đấu.");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    try {
      ...
      return await service.update(id, { ten_vi_tri: ten, trang_thai: nextVal });
    }
  }
  if (typeof service.put === "function") {
    try {
      ...
      return await service.put(id, { ten_vi_tri: ten, trang_thai: nextVal });
    }
  }
...
async function save() {
  ...
  if (!ten) return alert("Vui lòng nhập tên vị trí.");

  const ok = confirm(`Bạn có chắc chắn muốn thêm vị trí: "${ten}" không?`);
  if (!ok) return;

  saving.value = true;
  try {
    try {
      await (viTriThiDauService.create?.({ tenViTri: ten, trangThai: true, xoaMem: false }) ??
        viTriThiDauService.add?.({ tenViTri: ten, trangThai: true, xoaMem: false }));
    } catch (_) {
      await (viTriThiDauService.create?.({ ten_vi_tri: ten, trang_thai: true, xoa_mem: false }) ??
        viTriThiDauService.add?.({ ten_vi_tri: ten, trang_thai: true, xoa_mem: false }));
    }
...
function exportExcel() {
  ...
  rows.forEach((r, i) => {
    lines.push([String(i + 1), String(getMa(r) ?? ""), String(getTen(r) ?? ""), getTrangThai(r) ? "Đang hoạt động" : "Ngừng hoạt động"]);
  });
