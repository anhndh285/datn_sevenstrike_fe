<!-- src/pages/product/thuoc_tinh/ViTriThiDauPage.vue -->
<template>
  <div class="ss-page ss-font">
    <div class="ss-head">
      <div class="ss-head-left">
        <div class="ss-title">Quản lý Sản Phẩm/ Vị Trí Thi Đấu</div>
      </div>

      <div class="ss-head-right">
        <div class="ss-count">
          Tổng số vị trí thi đấu: <span>{{ totalCount }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="ss-card ss-border ss-filter-card">
      ...
      <div class="ss-filter-body">
        <div class="ss-filter-left">
          <div class="ss-filter-label">Tìm kiếm vị trí thi đấu</div>
    ...
    <div class="ss-card ss-border ss-list-card">
      <div class="ss-list-head">Danh sách vị trí thi đấu</div>
        ...
        <table class="table ss-table mb-0">
          <thead>
            <tr>
              ...
              <th class="col-ten">Tên vị trí</th>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">{{ modal.mode === 'create' ? 'Thêm vị trí thi đấu' : 'Chỉnh sửa vị trí thi đấu' }}</div>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <template v-if="modal.mode === 'edit'">
            <label class="form-label ss-label">Mã vị trí</label>
            <input :value="form.maViTri" class="form-control ss-input-readonly" readonly disabled />
          </template>

          <label class="form-label ss-label ss-label-gap">Tên vị trí *</label>
          <input
            v-model="form.tenViTri"
            class="form-control"
...
import viTriThiDauService from "@/services/thuoc_tinh/viTriThiDauService.js";
...
function getId(row) {
  return row?.id ?? row?.idViTri ?? row?.id_vi_tri ?? row?.viTriId ?? row?.vi_tri_id ?? null;
}

function getMa(x) {
  return x?.ma_vi_tri ?? x?.maViTri ?? x?.ma ?? "--";
}

function getTen(x) {
  return x?.ten_vi_tri ?? x?.tenViTri ?? x?.ten ?? "";
}
...
async function fetchAll() {
  ...
  try {
    const res =
      typeof viTriThiDauService.getAll === "function"
        ? await viTriThiDauService.getAll()
        : await viTriThiDauService.all?.();

    items.value = unwrapList(res);
  } catch (e) {
    console.error(e);
    showToast(getErrorMessage(e, "Không tải được danh sách vị trí thi đấu."), "error");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    return await service.update(id, { tenViTri: ten, trangThai: nextVal });
  }

  if (typeof service.put === "function") {
    return await service.put(id, { tenViTri: ten, trangThai: nextVal });
  }
...
async function onToggle(row) {
  ...
  try {
    await callUpdateTrangThai(viTriThiDauService, id, nextVal, row);
    await fetchAll();
    ...
    await Swal.fire({
      ...
      icon: "success",
      title: "Thành công!",
      text: "Đã cập nhật trạng thái vị trí thi đấu!",
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } catch (e) {
    ...
    await Swal.fire({
      ...
      icon: "error",
      title: "Thất bại!",
      text: getErrorMessage(e, "Không đổi được trạng thái vị trí thi đấu."),
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } finally {
...
function openEdit(row) {
  ...
  form.maViTri = getMa(row);
  form.tenViTri = getTen(row);
...
async function save() {
  const ten = (form.tenViTri || "").trim();
  modal.error = "";

  if (!ten) {
    modal.error = "Vui lòng nhập tên vị trí.";
    return;
  }
  ...
  try {
    if (modal.mode === "create") {
      await viTriThiDauService.create({
        tenViTri: ten,
        trangThai: true,
        xoaMem: false,
      });
      ...
      modal.open = false;
      await fetchAll();
      showToast("Thêm vị trí thi đấu thành công!");
      return;
    }

    if (modal.editingId === undefined || modal.editingId === null) {
      modal.error = "Không tìm thấy ID vị trí để cập nhật.";
      return;
    }

    if (typeof viTriThiDauService.update === "function") {
      await viTriThiDauService.update(modal.editingId, {
        tenViTri: ten,
        trangThai: form.trangThai,
      });
    } else if (typeof viTriThiDauService.put === "function") {
      await viTriThiDauService.put(modal.editingId, {
        tenViTri: ten,
        trangThai: form.trangThai,
      });
    } else {
      throw new Error("Service chưa có hàm cập nhật vị trí thi đấu");
    }

    modal.open = false;
    await fetchAll();
    showToast("Cập nhật vị trí thi đấu thành công!");
  } catch (e) {
    console.error(e);
    modal.error = getErrorMessage(
      e,
      modal.mode === "create" ? "Thêm vị trí thi đấu thất bại." : "Cập nhật vị trí thi đấu thất bại."
    );
  } finally {
    saving.value = false;
  }
...
function exportExcel() {
  ...
  const header = ["STT", "Mã", "Tên vị trí", "Trạng thái"];
  const lines = [header];
  ...
  a.href = url;
  a.download = `vi_tri_thi_dau_${new Date().toISOString().slice(0, 10)}.csv`;
