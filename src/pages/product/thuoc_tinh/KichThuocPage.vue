<!-- src/pages/product/thuoc_tinh/KichThuocPage.vue -->
<template>
  <div class="ss-page ss-font">
    <div class="ss-head">
      <div class="ss-head-left">
        <div class="ss-title">Quản lý Sản Phẩm/ Kích Thước</div>
      </div>

      <div class="ss-head-right">
        <div class="ss-count">
          Tổng số kích thước: <span>{{ totalCount }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="ss-card ss-border ss-filter-card">
      ...
      <div class="ss-filter-body">
        <div class="ss-filter-left">
          <div class="ss-filter-label">Tìm kiếm kích thước</div>
    ...
    <div class="ss-card ss-border ss-list-card">
      <div class="ss-list-head">Danh sách kích thước</div>
        ...
        <table class="table ss-table mb-0">
          <thead>
            <tr>
              ...
              <th class="col-ten">Tên kích thước</th>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">{{ modal.mode === 'create' ? 'Thêm kích thước' : 'Chỉnh sửa kích thước' }}</div>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <template v-if="modal.mode === 'edit'">
            <label class="form-label ss-label">Mã kích thước</label>
            <input :value="form.maKichThuoc" class="form-control ss-input-readonly" readonly disabled />
          </template>

          <label class="form-label ss-label ss-label-gap">Tên kích thước *</label>
          <input
...
import kichThuocService from "@/services/thuoc_tinh/kichThuocService.js";
...
function getId(row) {
  return row?.id ?? row?.idKichThuoc ?? row?.id_kich_thuoc ?? row?.kichThuocId ?? row?.kich_thuoc_id ?? null;
}

function getMa(x) {
  return x?.ma_kich_thuoc ?? x?.maKichThuoc ?? x?.ma ?? "--";
}

function getTen(x) {
  return x?.ten_kich_thuoc ?? x?.tenKichThuoc ?? x?.ten ?? "";
}
...
async function fetchAll() {
  ...
  try {
    const res =
      typeof kichThuocService.getAll === "function"
        ? await kichThuocService.getAll()
        : await kichThuocService.all?.();
    items.value = unwrapList(res);
  } catch (e) {
    console.error(e);
    showToast(getErrorMessage(e, "Không tải được danh sách kích thước."), "error");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    return await service.update(id, { tenKichThuoc: ten, trangThai: nextVal });
  }

  if (typeof service.put === "function") {
    return await service.put(id, { tenKichThuoc: ten, trangThai: nextVal });
  }
...
async function onToggle(row) {
  ...
  try {
    await callUpdateTrangThai(kichThuocService, id, nextVal, row);
    await fetchAll();
    ...
    await Swal.fire({
      ...
      icon: "success",
      title: "Thành công!",
      text: "Đã cập nhật trạng thái kích thước!",
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } catch (e) {
    ...
    await Swal.fire({
      ...
      icon: "error",
      title: "Thất bại!",
      text: getErrorMessage(e, "Không đổi được trạng thái kích thước."),
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } finally {
...
function openEdit(row) {
  ...
  form.maKichThuoc = getMa(row);
  form.tenKichThuoc = getTen(row);
...
async function save() {
  const ten = (form.tenKichThuoc || "").trim();
  modal.error = "";

  if (!ten) {
    modal.error = "Vui lòng nhập tên kích thước.";
    return;
  }
  ...
  try {
    ...
    if (modal.editingId === undefined || modal.editingId === null) {
      modal.error = "Không tìm thấy ID kích thước để cập nhật.";
      return;
    }

    if (typeof kichThuocService.update === "function") {
      await kichThuocService.update(modal.editingId, {
        tenKichThuoc: ten,
        trangThai: form.trangThai,
      });
    } else if (typeof kichThuocService.put === "function") {
      await kichThuocService.put(modal.editingId, {
        tenKichThuoc: ten,
        trangThai: form.trangThai,
      });
    } else {
      throw new Error("Service chưa có hàm cập nhật kích thước");
    }

    modal.open = false;
    await fetchAll();
    showToast("Cập nhật kích thước thành công!");
  } catch (e) {
    console.error(e);
    modal.error = getErrorMessage(
      e,
      modal.mode === "create" ? "Thêm kích thước thất bại." : "Cập nhật kích thước thất bại."
    );
  } finally {
    saving.value = false;
  }
...
function exportExcel() {
  ...
  const header = ["STT", "Mã", "Tên kích thước", "Trạng thái"];
  const lines = [header];
  ...
  a.href = url;
  a.download = `kich_thuoc_${new Date().toISOString().slice(0, 10)}.csv`;
