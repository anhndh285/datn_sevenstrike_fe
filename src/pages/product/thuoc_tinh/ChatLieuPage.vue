<!-- src/pages/product/thuoc_tinh/ChatLieuPage.vue -->
<template>
  <div class="ss-page ss-font">
    <div class="ss-head">
      <div class="ss-head-left">
        <div class="ss-title">Quản lý Sản Phẩm/ Chất Liệu</div>
      </div>

      <div class="ss-head-right">
        <div class="ss-count">
          Tổng số chất liệu: <span>{{ totalCount }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="ss-card ss-border ss-filter-card">
      ...
      <div class="ss-filter-body">
        <div class="ss-filter-left">
          <div class="ss-filter-label">Tìm kiếm chất liệu</div>
    ...
    <div class="ss-card ss-border ss-list-card">
      <div class="ss-list-head">Danh sách chất liệu</div>
        ...
        <table class="table ss-table mb-0">
          <thead>
            <tr>
              ...
              <th class="col-ten">Tên chất liệu</th>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">{{ modal.mode === 'create' ? 'Thêm chất liệu' : 'Chỉnh sửa chất liệu' }}</div>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <template v-if="modal.mode === 'edit'">
            <label class="form-label ss-label">Mã chất liệu</label>
            <input :value="form.maChatLieu" class="form-control ss-input-readonly" readonly disabled />
          </template>

          <label class="form-label ss-label ss-label-gap">Tên chất liệu *</label>
          <input
...
import chatLieuService from "@/services/thuoc_tinh/chatLieuService.js";
...
function getId(row) {
  return row?.id ?? row?.idChatLieu ?? row?.id_chat_lieu ?? row?.chatLieuId ?? row?.chat_lieu_id ?? null;
}

function getMa(x) {
  return x?.ma_chat_lieu ?? x?.maChatLieu ?? x?.ma ?? "--";
}

function getTen(x) {
  return x?.ten_chat_lieu ?? x?.tenChatLieu ?? x?.ten ?? "";
}
...
async function fetchAll() {
  ...
  try {
    const res =
      typeof chatLieuService.getAll === "function"
        ? await chatLieuService.getAll()
        : await chatLieuService.all?.();
    items.value = unwrapList(res);
  } catch (e) {
    console.error(e);
    showToast(getErrorMessage(e, "Không tải được danh sách chất liệu."), "error");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    return await service.update(id, { tenChatLieu: ten, trangThai: nextVal });
  }

  if (typeof service.put === "function") {
    return await service.put(id, { tenChatLieu: ten, trangThai: nextVal });
  }
...
async function onToggle(row) {
  ...
  try {
    await callUpdateTrangThai(chatLieuService, id, nextVal, row);
    await fetchAll();
    ...
    await Swal.fire({
      ...
      icon: "success",
      title: "Thành công!",
      text: "Đã cập nhật trạng thái chất liệu!",
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } catch (e) {
    ...
    await Swal.fire({
      ...
      icon: "error",
      title: "Thất bại!",
      text: getErrorMessage(e, "Không đổi được trạng thái chất liệu."),
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } finally {
...
function openEdit(row) {
  ...
  form.maChatLieu = getMa(row);
  form.tenChatLieu = getTen(row);
...
async function save() {
  const ten = (form.tenChatLieu || "").trim();
  modal.error = "";

  if (!ten) {
    modal.error = "Vui lòng nhập tên chất liệu.";
    return;
  }
  ...
  try {
    ...
    if (modal.editingId === undefined || modal.editingId === null) {
      modal.error = "Không tìm thấy ID chất liệu để cập nhật.";
      return;
    }

    if (typeof chatLieuService.update === "function") {
      await chatLieuService.update(modal.editingId, {
        tenChatLieu: ten,
        trangThai: form.trangThai,
      });
    } else if (typeof chatLieuService.put === "function") {
      await chatLieuService.put(modal.editingId, {
        tenChatLieu: ten,
        trangThai: form.trangThai,
      });
    } else {
      throw new Error("Service chưa có hàm cập nhật chất liệu");
    }

    modal.open = false;
    await fetchAll();
    showToast("Cập nhật chất liệu thành công!");
  } catch (e) {
    console.error(e);
    modal.error = getErrorMessage(
      e,
      modal.mode === "create" ? "Thêm chất liệu thất bại." : "Cập nhật chất liệu thất bại."
    );
  } finally {
    saving.value = false;
  }
...
function exportExcel() {
  ...
  const header = ["STT", "Mã", "Tên chất liệu", "Trạng thái"];
  const lines = [header];
  ...
  a.href = url;
  a.download = `chat_lieu_${new Date().toISOString().slice(0, 10)}.csv`;
