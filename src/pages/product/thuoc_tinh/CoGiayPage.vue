<!-- src/pages/product/thuoc_tinh/CoGiayPage.vue -->
<template>
  <div class="ss-page ss-font">
    <div class="ss-head">
      <div class="ss-head-left">
        <div class="ss-title">Quản lý Sản Phẩm/ Cổ Giày</div>
      </div>

      <div class="ss-head-right">
        <div class="ss-count">
          Tổng số cổ giày: <span>{{ totalCount }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="ss-card ss-border ss-filter-card">
      ...
      <div class="ss-filter-body">
        <div class="ss-filter-left">
          <div class="ss-filter-label">Tìm kiếm cổ giày</div>
    ...
    <div class="ss-card ss-border ss-list-card">
      <div class="ss-list-head">Danh sách cổ giày</div>
        ...
        <table class="table ss-table mb-0">
          <thead>
            <tr>
              ...
              <th class="col-ten">Tên cổ giày</th>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">{{ modal.mode === 'create' ? 'Thêm cổ giày' : 'Chỉnh sửa cổ giày' }}</div>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <template v-if="modal.mode === 'edit'">
            <label class="form-label ss-label">Mã cổ giày</label>
            <input :value="form.maCoGiay" class="form-control ss-input-readonly" readonly disabled />
          </template>

          <label class="form-label ss-label ss-label-gap">Tên cổ giày *</label>
          <input
            v-model="form.tenCoGiay"
            class="form-control"
...
function getId(row) {
  return row?.id ?? row?.idCoGiay ?? row?.id_co_giay ?? row?.coGiayId ?? row?.co_giay_id ?? null;
}

function getMa(x) {
  return x?.ma_co_giay ?? x?.maCoGiay ?? x?.ma ?? "--";
}

function getTen(x) {
  return x?.ten_co_giay ?? x?.tenCoGiay ?? x?.ten ?? "";
}
...
async function fetchAll() {
  ...
  try {
    ...
    console.error(e);
    showToast(getErrorMessage(e, "Không tải được danh sách cổ giày."), "error");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    return await service.update(id, { tenCoGiay: ten, trangThai: nextVal });
  }

  if (typeof service.put === "function") {
    return await service.put(id, { tenCoGiay: ten, trangThai: nextVal });
  }
...
async function onToggle(row) {
  ...
  try {
    await callUpdateTrangThai(coGiayService, id, nextVal, row);
    await fetchAll();
    ...
    await Swal.fire({
      ...
      icon: "success",
      title: "Thành công!",
      text: "Đã cập nhật trạng thái cổ giày!",
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } catch (e) {
    ...
    await Swal.fire({
      ...
      icon: "error",
      title: "Thất bại!",
      text: getErrorMessage(e, "Không đổi được trạng thái cổ giày."),
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } finally {
...
function openEdit(row) {
  ...
  form.maCoGiay = getMa(row);
  form.tenCoGiay = getTen(row);
...
async function save() {
  ...
  if (!ten) {
    modal.error = "Vui lòng nhập tên cổ giày.";
    return;
  }
  ...
  try {
    if (modal.mode === "create") {
      ...
      modal.open = false;
      await fetchAll();
      showToast("Thêm cổ giày thành công!");
      return;
    }

    if (modal.editingId === undefined || modal.editingId === null) {
      modal.error = "Không tìm thấy ID cổ giày để cập nhật.";
      return;
    }

    if (typeof coGiayService.update === "function") {
      await coGiayService.update(modal.editingId, {
        tenCoGiay: ten,
        trangThai: form.trangThai,
      });
    } else if (typeof coGiayService.put === "function") {
      await coGiayService.put(modal.editingId, {
        tenCoGiay: ten,
        trangThai: form.trangThai,
      });
    } else {
      throw new Error("Service chưa có hàm cập nhật cổ giày");
    }

    modal.open = false;
    await fetchAll();
    showToast("Cập nhật cổ giày thành công!");
  } catch (e) {
    console.error(e);
    modal.error = getErrorMessage(
      e,
      modal.mode === "create" ? "Thêm cổ giày thất bại." : "Cập nhật cổ giày thất bại."
    );
  } finally {
    saving.value = false;
  }
...
function exportExcel() {
  ...
  const header = ["STT", "Mã", "Tên cổ giày", "Trạng thái"];
  const lines = [header];
  ...
  a.href = url;
  a.download = `co_giay_${new Date().toISOString().slice(0, 10)}.csv`;
