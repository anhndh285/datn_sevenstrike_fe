      ...
      <div class="left-column">
        <div class="card info-card">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" disabled placeholder="Mã tự sinh (DGG-...)" style="background-color: #f1f5f9;" />
          </div>
          ...
          <div class="action-buttons mt-4">
            <button class="btn-update" @click="submitCreate" :disabled="isLoading">
                <i class="fa-solid fa-plus me-1"></i> {{ isLoading ? 'Đang xử lý...' : 'Tạo đợt giảm giá' }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="bottom-section">
      <div class="card detail-card">
        <div class="detail-header">
          <h3 class="section-title">Danh sách chi tiết sản phẩm được chọn
          ...
          <table class="custom-table">
            ...
            <tbody>
              ...
              <tr v-for="(item, index) in paginatedVariantsDisplay" :key="item.id">
                ...
                <td class="text-center font-weight-bold">{{ formatCurrency(item.giaNiemYet) }}</td>
...
const submitCreate = async () => {
  if (!formData.tenDotGiamGia || !formData.ngayBatDau || !formData.ngayKetThuc || formData.giaTriGiamGia === null) {
...
.btn-update { background: #16a34a; color: white; border: none; padding: 10px 20px; border-radius: 6px; font-weight: 600; cursor: pointer; display: flex; align-items: center; transition: 0.2s; flex: 1; justify-content: center; }
.btn-update:hover { background: #15803d; }
.btn-update:disabled { background: #86efac; cursor: not-allowed; }
