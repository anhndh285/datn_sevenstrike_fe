        ...
        <table class="custom-table">
          ...
          <tbody>
            ...
            <tr v-for="(item, index) in paginatedDiscounts" :key="item.id">
              ...
              <td class="text-center">
                <span class="discount-val-badge" :class="discountValClass(item.giaTriGiamGia)">
                  -{{ item.giaTriGiamGia }}%
                </span>
              </td>
...
const discountValClass = (val) => {
  const v = Number(val);
  if (v > 70) return "dv-green";
  if (v >= 50) return "dv-yellow";
  return "dv-red";
};
...
.dv-red    { background: rgba(239,68,68,0.1);   color: #dc2626; border-color: rgba(239,68,68,0.25); }
.dv-yellow { background: rgba(245,158,11,0.1);  color: #b45309; border-color: rgba(245,158,11,0.25); }
.dv-green  { background: rgba(34,197,94,0.1);   color: #16a34a; border-color: rgba(34,197,94,0.25); }
</style>
