<!-- src/pages/product/thuoc_tinh/ThuongHieuPage.vue -->
<template>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">{{ modal.mode === 'create' ? 'Thêm thương hiệu' : 'Chỉnh sửa thương hiệu' }}</div>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <template v-if="modal.mode === 'edit'">
            <label class="form-label ss-label">Mã thương hiệu</label>
            <input :value="form.maThuongHieu" class="form-control ss-input-readonly" readonly disabled />
          </template>

          <label class="form-label ss-label ss-label-gap">Tên thương hiệu *</label>
          <input
          ...
          <div v-if="modal.error" class="ss-form-error">
            {{ modal.error }}
          </div>
        </div>
...
function getId(row) {
  return row?.id ?? row?.idThuongHieu ?? row?.id_thuong_hieu ?? row?.thuongHieuId ?? row?.thuong_hieu_id ?? null;
}
...
async function fetchAll() {
  ...
  try {
    ...
    console.error(e);
    showToast(getErrorMessage(e, "Không tải được danh sách thương hiệu."), "error");
  } finally {
    loading.value = false;
  }
}
...
async function onToggle(row) {
  ...
  try {
    ...
    await Swal.fire({
      ...
      icon: "success",
      title: "Thành công!",
      text: "Đã cập nhật trạng thái thương hiệu!",
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } catch (e) {
    ...
    await Swal.fire({
      ...
      icon: "error",
      title: "Thất bại!",
      text: getErrorMessage(e, "Không đổi được trạng thái thương hiệu."),
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } finally {
...
function openEdit(row) {
  ...
  form.maThuongHieu = getMa(row);
  form.tenThuongHieu = getTen(row);
...
async function save() {
  ...
  if (!ten) {
    modal.error = "Vui lòng nhập tên thương hiệu.";
    return;
  }
  ...
  try {
    if (modal.mode === "create") {
      await thuongHieuService.create({
        tenThuongHieu: ten,
        trangThai: true,
        xoaMem: false,
      });
    ...
    if (modal.editingId === undefined || modal.editingId === null) {
      modal.error = "Không tìm thấy ID thương hiệu để cập nhật.";
      return;
    }

    if (typeof thuongHieuService.update === "function") {
      await thuongHieuService.update(modal.editingId, {
        tenThuongHieu: ten,
        trangThai: form.trangThai,
      });
    } else if (typeof thuongHieuService.put === "function") {
      await thuongHieuService.put(modal.editingId, {
        tenThuongHieu: ten,
        trangThai: form.trangThai,
      });
    } else {
      throw new Error("Service chưa có hàm cập nhật thương hiệu");
    }
    ...
    modal.error = getErrorMessage(
      e,
      modal.mode === "create" ? "Thêm thương hiệu thất bại." : "Cập nhật thương hiệu thất bại."
    );
  } finally {
    saving.value = false;
  }
