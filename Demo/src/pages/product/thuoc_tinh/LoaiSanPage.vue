<!-- src/pages/product/thuoc_tinh/LoaiSanPage.vue -->
<template>
  <div class="ss-page ss-font">
    <div class="ss-head">
      <div class="ss-head-left">
        <div class="ss-title">Quản lý Sản Phẩm/ Loại Sân</div>
      </div>

      <div class="ss-head-right">
        <div class="ss-count">
          Tổng số loại sân: <span>{{ totalCount }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="ss-card ss-border ss-filter-card">
      ...
      <div class="ss-filter-body">
        <div class="ss-filter-left">
          <div class="ss-filter-label">Tìm kiếm loại sân</div>
    ...
    <div class="ss-card ss-border ss-list-card">
      <div class="ss-list-head">Danh sách loại sân</div>
        ...
        <table class="table ss-table mb-0">
          <thead>
            <tr>
              ...
              <th class="col-ten">Tên loại sân</th>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">{{ modal.mode === 'create' ? 'Thêm loại sân' : 'Chỉnh sửa loại sân' }}</div>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <template v-if="modal.mode === 'edit'">
            <label class="form-label ss-label">Mã loại sân</label>
            <input :value="form.maLoaiSan" class="form-control ss-input-readonly" readonly disabled />
          </template>

          <label class="form-label ss-label ss-label-gap">Tên loại sân *</label>
          <input
            v-model="form.tenLoaiSan"
            class="form-control"
...
import loaiSanService from "@/services/thuoc_tinh/loaiSanService.js";
...
function getId(row) {
  return row?.id ?? row?.idLoaiSan ?? row?.id_loai_san ?? row?.loaiSanId ?? row?.loai_san_id ?? null;
}

function getMa(x) {
  return x?.ma_loai_san ?? x?.maLoaiSan ?? x?.ma ?? "--";
}

function getTen(x) {
  return x?.ten_loai_san ?? x?.tenLoaiSan ?? x?.ten ?? "";
}
...
async function fetchAll() {
  ...
  try {
    const res =
      typeof loaiSanService.getAll === "function"
        ? await loaiSanService.getAll()
        : await loaiSanService.all?.();
    items.value = unwrapList(res);
  } catch (e) {
    console.error(e);
    showToast(getErrorMessage(e, "Không tải được danh sách loại sân."), "error");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    return await service.update(id, { tenLoaiSan: ten, trangThai: nextVal });
  }

  if (typeof service.put === "function") {
    return await service.put(id, { tenLoaiSan: ten, trangThai: nextVal });
  }
...
async function onToggle(row) {
  ...
  try {
    await callUpdateTrangThai(loaiSanService, id, nextVal, row);
    await fetchAll();
    ...
    await Swal.fire({
      ...
      icon: "success",
      title: "Thành công!",
      text: "Đã cập nhật trạng thái loại sân!",
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } catch (e) {
    ...
    await Swal.fire({
      ...
      icon: "error",
      title: "Thất bại!",
      text: getErrorMessage(e, "Không đổi được trạng thái loại sân."),
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } finally {
...
function openEdit(row) {
  ...
  form.maLoaiSan = getMa(row);
  form.tenLoaiSan = getTen(row);
...
async function save() {
  const ten = (form.tenLoaiSan || "").trim();
  modal.error = "";

  if (!ten) {
    modal.error = "Vui lòng nhập tên loại sân.";
    return;
  }
  ...
  try {
    if (modal.mode === "create") {
      ...
      modal.open = false;
      await fetchAll();
      showToast("Thêm loại sân thành công!");
      return;
    }

    if (modal.editingId === undefined || modal.editingId === null) {
      modal.error = "Không tìm thấy ID loại sân để cập nhật.";
      return;
    }

    if (typeof loaiSanService.update === "function") {
      await loaiSanService.update(modal.editingId, {
        tenLoaiSan: ten,
        trangThai: form.trangThai,
      });
    } else if (typeof loaiSanService.put === "function") {
      await loaiSanService.put(modal.editingId, {
        tenLoaiSan: ten,
        trangThai: form.trangThai,
      });
    } else {
      throw new Error("Service chưa có hàm cập nhật loại sân");
    }

    modal.open = false;
    await fetchAll();
    showToast("Cập nhật loại sân thành công!");
  } catch (e) {
    console.error(e);
    modal.error = getErrorMessage(
      e,
      modal.mode === "create" ? "Thêm loại sân thất bại." : "Cập nhật loại sân thất bại."
    );
  } finally {
    saving.value = false;
  }
...
function exportExcel() {
  ...
  const header = ["STT", "Mã", "Tên loại sân", "Trạng thái"];
  const lines = [header];
  ...
  a.href = url;
  a.download = `loai_san_${new Date().toISOString().slice(0, 10)}.csv`;
