<!-- File: src/pages/product/thuoc_tinh/CoGiayPage.vue -->
<template>
  <div class="ss-page ss-font">
      ...
      <div class="table-responsive">
        <table class="table ss-table mb-0">
          ...
          <tbody v-else-if="loading">
            <tr><td colspan="4" class="text-center py-4 ss-muted">Đang tải...</td></tr>
          </tbody>

          <tbody v-else>
            <tr><td colspan="4" class="text-center py-4 ss-muted">Không có dữ liệu</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">Thêm cổ giày</div>
          <button class="btn btn-sm btn-outline-secondary" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <label class="form-label ss-label">Tên cổ giày *</label>
    ...
    <div v-if="success.open" class="ss-overlay">
      <div class="ss-success">
        ...
        <div class="ss-muted">Thêm cổ giày thành công!</div>
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    try {
      ...
      return await service.update(id, { ten_co_giay: ten, trang_thai: nextVal });
    }
  }
  if (typeof service.put === "function") {
    try {
      ...
      return await service.put(id, { ten_co_giay: ten, trang_thai: nextVal });
    }
  }
...
async function save() {
  ...
  try {
    try {
      await (coGiayService.create?.({ tenCoGiay: ten, trangThai: true, xoaMem: false }) ??
        coGiayService.add?.({ tenCoGiay: ten, trangThai: true, xoaMem: false }));
    } catch (_) {
      await (coGiayService.create?.({ ten_co_giay: ten, trang_thai: true, xoa_mem: false }) ??
        coGiayService.add?.({ ten_co_giay: ten, trang_thai: true, xoa_mem: false }));
    }
...
function exportExcel() {
  ...
  const header = ["STT", "Mã", "Tên cổ giày"];
  const lines = [header];

  rows.forEach((r, i) => {
    lines.push([String(i + 1), String(getMa(r) ?? ""), String(getTen(r) ?? "")]);
  });
...
.col-stt { width: 20%; }
.col-ma { width: 20%; }
.col-ten { width: 40%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.col-action { width: 20%; }
