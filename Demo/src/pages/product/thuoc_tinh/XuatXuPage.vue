    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">{{ modal.mode === 'create' ? 'Thêm xuất xứ' : 'Chỉnh sửa xuất xứ' }}</div>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <template v-if="modal.mode === 'edit'">
            <label class="form-label ss-label">Mã xuất xứ</label>
            <input
              :value="form.maXuatXu"
              class="form-control ss-input-readonly"
              readonly
              disabled
            />
          </template>

          <label class="form-label ss-label ss-label-gap">
            Tên xuất xứ *
          </label>
          <input
            v-model="form.tenXuatXu"
            class="form-control"
...
async function fetchAll() {
  ...
  try {
    const res =
      typeof xuatXuService.getAll === "function"
        ? await xuatXuService.getAll()
        : await xuatXuService.all?.();

    items.value = unwrapList(res);
  } catch (e) {
    console.error(e);
    showToast(getErrorMessage(e, "Không tải được danh sách xuất xứ."), "error");
  } finally {
    loading.value = false;
  }
}
...
async function onToggle(row) {
  ...
  try {
    ...
    await Swal.fire({
      ...
      icon: "success",
      title: "Thành công!",
      text: "Đã cập nhật trạng thái xuất xứ!",
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } catch (e) {
    ...
    await Swal.fire({
      ...
      icon: "error",
      title: "Thất bại!",
      text: getErrorMessage(e, "Không đổi được trạng thái xuất xứ."),
      confirmButtonText: "OK",
      showCancelButton: false,
    });
  } finally {
...
function openEdit(row) {
  ...
  form.maXuatXu = getMa(row);
  form.tenXuatXu = getTen(row);
...
async function save() {
  ...
  if (!ten) {
    modal.error = "Vui lòng nhập tên xuất xứ.";
    return;
  }
  ...
  try {
    if (modal.mode === "create") {
      ...
      modal.open = false;
      await fetchAll();
      showToast("Thêm xuất xứ thành công!");
      return;
    }

    if (modal.editingId === undefined || modal.editingId === null) {
      modal.error = "Không tìm thấy ID xuất xứ để cập nhật.";
      return;
    }

    if (typeof xuatXuService.update === "function") {
      await xuatXuService.update(modal.editingId, {
        tenXuatXu: ten,
        trangThai: form.trangThai,
      });
    } else if (typeof xuatXuService.put === "function") {
      await xuatXuService.put(modal.editingId, {
        tenXuatXu: ten,
        trangThai: form.trangThai,
      });
    } else {
      throw new Error("Service chưa có hàm cập nhật xuất xứ");
    }

    modal.open = false;
    await fetchAll();
    showToast("Cập nhật xuất xứ thành công!");
  } catch (e) {
    console.error(e);
    modal.error = getErrorMessage(
      e,
      modal.mode === "create" ? "Thêm xuất xứ thất bại." : "Cập nhật xuất xứ thất bại."
    );
  } finally {
    saving.value = false;
  }
