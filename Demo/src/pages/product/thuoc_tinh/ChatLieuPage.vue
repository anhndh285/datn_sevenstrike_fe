<!-- File: src/pages/product/thuoc_tinh/ChatLieuPage.vue -->
<template>
  <div class="ss-page ss-font">
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">Thêm chất liệu</div>
          <button class="btn btn-sm btn-outline-secondary" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <label class="form-label ss-label">Tên chất liệu *</label>
          <input v-model="form.tenChatLieu" class="form-control" placeholder="Nhập tên..." />
        </div>
    ...
    <div v-if="success.open" class="ss-overlay">
      <div class="ss-success">
        ...
        <div class="ss-muted">Thêm chất liệu thành công!</div>
...
async function fetchAll() {
  ...
  try {
    ...
    console.error(e);
    alert("Không tải được danh sách chất liệu.");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    try {
      ...
      return await service.update(id, { ten_chat_lieu: ten, trang_thai: nextVal });
    }
  }
  if (typeof service.put === "function") {
    try {
      ...
      return await service.put(id, { ten_chat_lieu: ten, trang_thai: nextVal });
    }
  }
...
const form = reactive({ tenChatLieu: "" });
...
async function save() {
  ...
  if (!ten) return alert("Vui lòng nhập tên chất liệu.");

  const ok = confirm(`Bạn có chắc chắn muốn thêm chất liệu: "${ten}" không?`);
  if (!ok) return;

  saving.value = true;
  try {
    try {
      await (chatLieuService.create?.({ tenChatLieu: ten, trangThai: true, xoaMem: false }) ??
        chatLieuService.add?.({ tenChatLieu: ten, trangThai: true, xoaMem: false }));
    } catch (_) {
      await (chatLieuService.create?.({ ten_chat_lieu: ten, trang_thai: true, xoa_mem: false }) ??
        chatLieuService.add?.({ ten_chat_lieu: ten, trang_thai: true, xoa_mem: false }));
    }
