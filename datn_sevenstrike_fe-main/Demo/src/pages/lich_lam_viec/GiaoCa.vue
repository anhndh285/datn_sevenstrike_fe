            ...
            <div class="ho-header">
                <div class="ho-header-left">
                    ...
                    <div class="ho-header-text">
                        ...
                        <span class="ho-sub-text">#{{ caHienTai.id ? caHienTai.id.toString().substring(0,8) : 'cccadd2e' }} • {{ formattedFullTime }}</span>
                    </div>
                </div>
            ...
            <div class="ho-body">
                <div class="ho-card ho-left-col">
                    ...
                    <div class="ho-row">
                        ...
                        <span class="ho-val-text text-primary font-bold">0 ₫</span> </div>

                    <div class="ho-divider">
                        <i class="fa-solid fa-receipt"></i> Đã thanh toán: <strong>{{ caHienTai.soDonHangDaThanhToan || 0 }}</strong> đơn
                    </div>
                ...
                <div class="ho-card ho-right-col">
                    ...
                    <div class="form-group mt-3">
                        ...
                        <div class="ho-input-wrapper">
                            ...
                            <input type="text" class="ho-input-money" 
...
// Mặc định tiền thực tế là 0 để hiển thị số âm (thiếu hụt) nếu chưa nhập
...
const handleBatDauCa = async () => {
    ...
    try {
        ...
        errorMessage.value = error.response?.data?.message || "Không thể bắt đầu ca vào lúc này.";
    } finally {
...
const submitKetThucCa = async () => {
    ...
    // Yêu cầu nhập nếu logic của bạn bắt buộc phải có giải trình khi chênh lệch
    ...
    try {
        ...
        // Tự động đăng xuất theo thiết kế
        handleLogout();
    } catch (error) {
...
.ho-icon-box {
    background-color: #e6f7f2; color: #10b981;
    width: 40px; height: 40px; border-radius: 8px;
    display: flex; justify-content: center; align-items: center;
    font-size: 1.2rem;
}
.ho-header-text h2 { margin: 0; font-size: 1.2rem; font-weight: 700; color: #111827; }
...
.ho-badge {
    ...
    align-items: flex-end; border: 1px solid #f3f4f6;
}
.ho-role { font-size: 0.7rem; font-weight: 700; color: #9ca3af; letter-spacing: 0.5px; }
.ho-name { font-size: 0.95rem; font-weight: 700; color: #111827; }
...
.text-success-icon { color: #10b981; font-size: 1.2rem; }
...
.ho-divider {
    ...
    padding: 16px 0; border-top: 1px dashed #e5e7eb; border-bottom: 1px dashed #e5e7eb;
...
.ho-summary-title { color: #059669; font-weight: 700; font-size: 0.85rem; }
.ho-summary-value { color: #059669; font-size: 2rem; font-weight: 800; }
.ho-summary-note { color: #34d399; font-size: 0.8rem; }

/* Right Col - Kiểm kê */
.ho-input-label { font-size: 0.8rem; font-weight: 700; color: #9ca3af; margin-bottom: 8px; display: block; }
.ho-input-wrapper { position: relative; display: flex; align-items: center; }
.ho-currency { position: absolute; left: 15px; color: #9ca3af; font-weight: 700; font-size: 1.2rem; }
.ho-currency-right { position: absolute; right: 15px; color: #9ca3af; font-weight: 700; font-size: 1.2rem; }
.ho-input-money {
    width: 100%; padding: 14px 40px 14px 40px; font-size: 1.2rem;
    font-weight: 700; border: 1px solid #10b981; border-radius: 8px; text-align: right;
    color: #111827; transition: all 0.2s;
}
.ho-input-money:focus { outline: none; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1); }

.ho-diff-box {
    display: flex; justify-content: space-between; align-items: center;
    padding: 12px 16px; border-radius: 8px; margin-top: 15px; font-weight: 700;
}
...
.ho-textarea {
    ...
    resize: none; font-family: inherit; font-size: 0.95rem;
}
.ho-textarea:focus { outline: none; border-color: #10b981; }

.btn-submit-end {
    width: 100%; background: #10b981; color: white; border: none;
    padding: 15px; border-radius: 8px; font-weight: 700; font-size: 1.05rem;
    cursor: pointer; margin-top: 25px; transition: 0.2s;
}
.btn-submit-end:hover { background: #059669; }
...
/* Các components từ Modal Mở ca & Component Base khác giữ nguyên */
.modal-overlay {
...
.ss-schedule-box { background-color: #f0fdf4; border-radius: 8px; padding: 12px 16px; margin-bottom: 15px; }
.ss-schedule-label { font-size: 0.75rem; font-weight: 700; color: #059669; margin-bottom: 6px; }
...
.ss-shift-time { background-color: #d1fae5; color: #047857; padding: 4px 12px; border-radius: 20px; font-weight: 600; font-size: 0.85rem; }
...
.money-input-new { width: 100%; padding: 12px 15px 12px 35px; font-size: 1.25rem; font-weight: 700; border: 1px solid #d1d5db; border-radius: 8px; color: #111827; }
.money-input-new:focus, .note-textarea:focus { outline: none; border-color: #10b981; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1); }
