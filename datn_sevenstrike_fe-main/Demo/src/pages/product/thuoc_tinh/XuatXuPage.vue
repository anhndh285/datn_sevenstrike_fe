<!-- File: src/pages/product/thuoc_tinh/XuatXuPage.vue -->
<template>
  <div class="ss-page ss-font">
    <!-- HEAD (nhỏ + chữ thường + không đậm) + tổng số ngoài bộ lọc -->
    <div class="ss-head">
      <div class="ss-head-left">
        <div class="ss-title">Quản lý Sản Phẩm/ Xuất Xứ</div>
      </div>

      <div class="ss-head-right">
        <div class="ss-count">
          Tổng số xuất xứ: <span>{{ totalCount }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="ss-card ss-border ss-filter-card">
      ...
      <div class="ss-filter-body">
        <div class="ss-filter-left">
          <div class="ss-filter-label">Tìm kiếm xuất xứ</div>
    ...
    <div class="ss-card ss-border ss-list-card">
      <div class="ss-list-head">Danh sách xuất xứ</div>
        ...
        <table class="table ss-table mb-0">
          <thead>
            <tr>
              ...
              <th class="col-ten">Tên xuất xứ</th>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">Thêm xuất xứ</div>
          <button class="btn btn-sm btn-outline-secondary" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <label class="form-label ss-label">Tên xuất xứ *</label>
          <input v-model="form.tenXuatXu" class="form-control" placeholder="Nhập tên..." />
        </div>
    ...
    <div v-if="success.open" class="ss-overlay">
      <div class="ss-success">
        ...
        <div class="ss-muted">Thêm xuất xứ thành công!</div>
...
import xuatXuService from "@/services/thuoc_tinh/xuatXuService.js";
...
function getId(row) {
  return row?.id ?? row?.idXuatXu ?? row?.id_xuat_xu ?? row?.xuatXuId ?? row?.xuat_xu_id ?? null;
}

function getMa(x) {
  return x?.ma_xuat_xu ?? x?.maXuatXu ?? x?.ma ?? "--";
}
function getTen(x) {
  return x?.ten_xuat_xu ?? x?.tenXuatXu ?? x?.ten ?? "";
}
...
async function fetchAll() {
  ...
  try {
    const res =
      typeof xuatXuService.getAll === "function" ? await xuatXuService.getAll() : await xuatXuService.all?.();
    items.value = unwrapList(res);
  } catch (e) {
    console.error(e);
    alert("Không tải được danh sách xuất xứ.");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    try {
      return await service.update(id, { tenXuatXu: ten, trangThai: nextVal });
    } catch (_) {
      return await service.update(id, { ten_xuat_xu: ten, trang_thai: nextVal });
    }
  }
  if (typeof service.put === "function") {
    try {
      return await service.put(id, { tenXuatXu: ten, trangThai: nextVal });
    } catch (_) {
      return await service.put(id, { ten_xuat_xu: ten, trang_thai: nextVal });
    }
  }
...
async function onToggle(row) {
  ...
  try {
    await callUpdateTrangThai(xuatXuService, id, nextVal, row);
    await fetchAll();
  } catch (e) {
    console.error(e);
...
async function save() {
  const ten = (form.tenXuatXu || "").trim();
  if (!ten) return alert("Vui lòng nhập tên xuất xứ.");

  const ok = confirm(`Bạn có chắc chắn muốn thêm xuất xứ: "${ten}" không?`);
  if (!ok) return;

  saving.value = true;
  try {
    try {
      await (xuatXuService.create?.({ tenXuatXu: ten, trangThai: true, xoaMem: false }) ??
        xuatXuService.add?.({ tenXuatXu: ten, trangThai: true, xoaMem: false }));
    } catch (_) {
      await (xuatXuService.create?.({ ten_xuat_xu: ten, trang_thai: true, xoa_mem: false }) ??
        xuatXuService.add?.({ ten_xuat_xu: ten, trang_thai: true, xoa_mem: false }));
    }
...
function exportExcel() {
  ...
  const header = ["STT", "Mã", "Tên xuất xứ", "Trạng thái"];
  const lines = [header];
  ...
  a.href = url;
  a.download = `xuat_xu_${new Date().toISOString().slice(0, 10)}.csv`;
...
<style scoped>
/* y hệt style bạn đang dùng trong ThuongHieuPage.vue */
