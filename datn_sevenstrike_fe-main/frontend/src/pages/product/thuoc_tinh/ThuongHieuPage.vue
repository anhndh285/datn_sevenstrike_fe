<!-- File: src/pages/product/thuoc_tinh/ThuongHieuPage.vue -->
<template>
  <div class="ss-page ss-font">
    <!-- HEAD (nhỏ + chữ thường + không đậm) + tổng số ngoài bộ lọc -->
    <div class="ss-head">
      <div class="ss-head-left">
        <div class="ss-title">Quản lý Sản Phẩm/ Thương Hiệu</div>
      </div>

      <div class="ss-head-right">
        <div class="ss-count">
          Tổng số thương hiệu: <span>{{ totalCount }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="ss-card ss-border ss-filter-card">
      ...
      <div class="ss-filter-body">
        <div class="ss-filter-left">
          <div class="ss-filter-label">Tìm kiếm thương hiệu</div>
    ...
    <div class="ss-card ss-border ss-list-card">
      <div class="ss-list-head">Danh sách thương hiệu</div>
        ...
        <table class="table ss-table mb-0">
          <thead>
            <tr>
              ...
              <th class="col-ten">Tên thương hiệu</th>
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">Thêm thương hiệu</div>
          <button class="btn btn-sm btn-outline-secondary" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <label class="form-label ss-label">Tên thương hiệu *</label>
          <input v-model="form.tenThuongHieu" class="form-control" placeholder="Nhập tên..." />
        </div>
    ...
    <div v-if="success.open" class="ss-overlay">
      <div class="ss-success">
        ...
        <div class="ss-muted">Thêm thương hiệu thành công!</div>
...
import thuongHieuService from "@/services/thuoc_tinh/thuongHieuService.js";
...
function getMa(x) {
  return x?.ma_thuong_hieu ?? x?.maThuongHieu ?? x?.ma ?? "--";
}
function getTen(x) {
  return x?.ten_thuong_hieu ?? x?.tenThuongHieu ?? x?.ten ?? "";
}
...
async function fetchAll() {
  ...
  try {
    const res =
      typeof thuongHieuService.getAll === "function"
        ? await thuongHieuService.getAll()
        : await thuongHieuService.all?.();
...
/**
 * ✅ FIX: BE thường validate tên NOT NULL khi update,
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    try {
      return await service.update(id, { tenThuongHieu: ten, trangThai: nextVal });
    } catch (_) {
      return await service.update(id, { ten_thuong_hieu: ten, trang_thai: nextVal });
    }
  }
  if (typeof service.put === "function") {
    try {
      return await service.put(id, { tenThuongHieu: ten, trangThai: nextVal });
    } catch (_) {
      return await service.put(id, { ten_thuong_hieu: ten, trang_thai: nextVal });
    }
  }
...
async function onToggle(row) {
  ...
  try {
    await callUpdateTrangThai(thuongHieuService, id, nextVal, row);
    // nếu BE trả list khác format, reload nhẹ
    await fetchAll();
  } catch (e) {
    console.error(e);
...
const form = reactive({ tenThuongHieu: "" });
...
async function save() {
  const ten = (form.tenThuongHieu || "").trim();
  if (!ten) return alert("Vui lòng nhập tên thương hiệu.");

  const ok = confirm(`Bạn có chắc chắn muốn thêm thương hiệu: "${ten}" không?`);
  if (!ok) return;

  saving.value = true;
  try {
    try {
      await (thuongHieuService.create?.({ tenThuongHieu: ten, trangThai: true, xoaMem: false }) ??
        thuongHieuService.add?.({ tenThuongHieu: ten, trangThai: true, xoaMem: false }));
    } catch (_) {
      await (thuongHieuService.create?.({ ten_thuong_hieu: ten, trang_thai: true, xoa_mem: false }) ??
        thuongHieuService.add?.({ ten_thuong_hieu: ten, trang_thai: true, xoa_mem: false }));
    }
...
/**
 * "Tải Excel" (không thêm thư viện): xuất CSV UTF-8 BOM để mở bằng Excel.
 */
function exportExcel() {
  const rows = filteredItems.value || [];
  const header = ["STT", "Mã", "Tên thương hiệu", "Trạng thái"];
  const lines = [header];
  ...
  a.href = url;
  a.download = `thuong_hieu_${new Date().toISOString().slice(0, 10)}.csv`;
...
.ss-title {
  ...
  font-weight: 600;       /* vừa phải, không chói */
...
.ss-filter-card {
  padding: 14px 16px;
  margin-bottom: 18px; /* ✅ tạo khoảng cách, không dính */
}
.ss-filter-head {
  display: inline-flex;
  align-items: center;
...
/* Thêm thương hiệu - gradient logo */
.ss-btn-primary {
