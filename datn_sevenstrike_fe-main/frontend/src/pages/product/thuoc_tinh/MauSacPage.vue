<!-- File: src/pages/product/thuoc_tinh/MauSacPage.vue -->
<template>
  <div class="ss-page ss-font">
    ...
    <!-- Modal create (GIỮ chức năng riêng của màu sắc: nhập HEX + swatch preview) -->
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">Thêm màu sắc</div>
          <button class="btn btn-sm btn-outline-secondary" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <div class="mb-3">
            <label class="form-label ss-label">Tên màu sắc *</label>
            <input v-model="form.tenMauSac" class="form-control" placeholder="Nhập tên..." />
          </div>

          <div>
            <label class="form-label ss-label">Mã màu (HEX) *</label>
            <div class="d-flex align-items-center gap-2">
              <span
              ...
              <input v-model="form.maMauHex" class="form-control" placeholder="Ví dụ: #111827 hoặc #ff4d4f" />
            </div>
            <div class="ss-muted mt-1">Nhập dạng #RRGGBB (khuyến nghị) hoặc #RGB.</div>
          </div>
        </div>
    ...
    <div v-if="success.open" class="ss-overlay">
      <div class="ss-success">
        ...
        <div class="ss-muted">Thêm màu sắc thành công!</div>
...
/**
 * ✅ Mã màu HEX theo BE: maMauHex (fallback thêm snake_case)
 */
function getMaMau(x) {
  return (
    x?.maMauHex ??
...
async function fetchAll() {
  ...
  try {
    ...
    console.error(e);
    alert("Không tải được danh sách màu sắc.");
  } finally {
    loading.value = false;
  }
}
...
function onView(row) {
  alert(
    `Mã: ${getMa(row)}\nTên: ${getTen(row)}\nMã màu: ${normalizeHex(getMaMau(row)) || "--"}\nTrạng thái: ${
      getTrangThai(row) ? "Đang hoạt động" : "Ngừng hoạt động"
    }`
  );
}
...
/**
 * ✅ FIX giống các màn khác: khi BE validate NOT NULL, đổi trạng thái phải gửi kèm tên + maMauHex
 */
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  // fallback PUT/UPDATE: gửi kèm dữ liệu bắt buộc
...
// Modal + create (GIỮ key BE: maMauHex)
const modal = reactive({ open: false });
const success = reactive({ open: false });
const form = reactive({ tenMauSac: "", maMauHex: "" });
...
async function save() {
  ...
  if (!ten) return alert("Vui lòng nhập tên màu sắc.");
  if (!maMauHex) return alert("Vui lòng nhập mã màu HEX hợp lệ (VD: #111827 hoặc #ff4d4f).");

  const ok = confirm(`Bạn có chắc chắn muốn thêm màu sắc: "${ten}" không?`);
  if (!ok) return;

  saving.value = true;
  try {
    await mauSacService.create({ tenMauSac: ten, maMauHex, trangThai: true, xoaMem: false });
