<!-- File: src/pages/product/thuoc_tinh/LoaiSanPage.vue -->
<template>
  <div class="ss-page ss-font">
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">Thêm loại sân</div>
          <button class="btn btn-sm btn-outline-secondary" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <label class="form-label ss-label">Tên loại sân *</label>
          <input v-model="form.tenLoaiSan" class="form-control" placeholder="Nhập tên..." />
        </div>
    ...
    <div v-if="success.open" class="ss-overlay">
      <div class="ss-success">
        ...
        <div class="ss-muted">Thêm loại sân thành công!</div>
...
function getMa(x) {
  return x?.ma_loai_san ?? x?.maLoaiSan ?? x?.ma ?? x?.code ?? "--";
}
function getTen(x) {
  return x?.ten_loai_san ?? x?.tenLoaiSan ?? x?.ten ?? x?.name ?? "";
}
...
async function fetchAll() {
  ...
  try {
    ...
    console.error(e);
    alert("Không tải được danh sách loại sân.");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    try {
      ...
      return await service.update(id, { ten_loai_san: ten, trang_thai: nextVal });
    }
  }
  if (typeof service.put === "function") {
    try {
      ...
      return await service.put(id, { ten_loai_san: ten, trang_thai: nextVal });
    }
  }
...
const form = reactive({ tenLoaiSan: "" });
...
async function save() {
  ...
  if (!ten) return alert("Vui lòng nhập tên loại sân.");

  const ok = confirm(`Bạn có chắc chắn muốn thêm loại sân: "${ten}" không?`);
  if (!ok) return;

  saving.value = true;
  try {
    try {
      await (loaiSanService.create?.({ tenLoaiSan: ten, trangThai: true, xoaMem: false }) ??
        loaiSanService.add?.({ tenLoaiSan: ten, trangThai: true, xoaMem: false }));
    } catch (_) {
      await (loaiSanService.create?.({ ten_loai_san: ten, trang_thai: true, xoa_mem: false }) ??
        loaiSanService.add?.({ ten_loai_san: ten, trang_thai: true, xoa_mem: false }));
    }
