<!-- File: src/pages/product/thuoc_tinh/KichThuocPage.vue -->
<template>
  <div class="ss-page ss-font">
    ...
    <div v-if="modal.open" class="ss-overlay">
      <div class="ss-modal">
        <div class="ss-modal-header">
          <div class="ss-modal-title">Thêm kích thước</div>
          <button class="btn btn-sm btn-outline-secondary" @click="closeModal">X</button>
        </div>

        <div class="ss-modal-body">
          <label class="form-label ss-label">Tên kích thước *</label>
          <input v-model="form.tenKichThuoc" class="form-control" placeholder="Nhập tên..." />
        </div>
    ...
    <div v-if="success.open" class="ss-overlay">
      <div class="ss-success">
        ...
        <div class="ss-muted">Thêm kích thước thành công!</div>
...
async function fetchAll() {
  ...
  try {
    ...
    console.error(e);
    alert("Không tải được danh sách kích thước.");
  } finally {
    loading.value = false;
  }
}
...
async function callUpdateTrangThai(service, id, nextVal, row) {
  ...
  if (typeof service.update === "function") {
    try {
      ...
      return await service.update(id, { ten_kich_thuoc: ten, trang_thai: nextVal });
    }
  }
  if (typeof service.put === "function") {
    try {
      ...
      return await service.put(id, { ten_kich_thuoc: ten, trang_thai: nextVal });
    }
  }
...
const form = reactive({ tenKichThuoc: "" });
...
async function save() {
  ...
  if (!ten) return alert("Vui lòng nhập tên kích thước.");

  const ok = confirm(`Bạn có chắc chắn muốn thêm kích thước: "${ten}" không?`);
  if (!ok) return;

  saving.value = true;
  try {
    try {
      await (kichThuocService.create?.({ tenKichThuoc: ten, trangThai: true, xoaMem: false }) ??
        kichThuocService.add?.({ tenKichThuoc: ten, trangThai: true, xoaMem: false }));
    } catch (_) {
      await (kichThuocService.create?.({ ten_kich_thuoc: ten, trang_thai: true, xoa_mem: false }) ??
        kichThuocService.add?.({ ten_kich_thuoc: ten, trang_thai: true, xoa_mem: false }));
    }
